...
def eval(model, train_dataloader, all_labels, device, tokenizer):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(train_dataloader):
            # mask从0到句子结束是True，后面zeroPad
            tokens, masks = [x.to(device) for x in batch]
            tokens, _, _ = get_batch(tokens, tokenizer, batchsize=16)
            output = model(tokens)
...
if __name__ == '__main__':
    ...
    state_dict = torch.load('../models/save_distill.pth', map_location='cpu')
    ...
    train_dataloader, train_labels = load_tnews_data('../nlpdata/tnews_public', 'dev', tokenizer)

    loss_fcn = nn.CrossEntropyLoss()
    loss_fcn.to(device)

    eval(model, train_dataloader, train_labels, device, tokenizer)